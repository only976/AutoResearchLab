...
| Agent | 职责 | 触发入口 | 模式 |
|-------|------|----------|------|
| **Idea Agent** | 从模糊 idea 提取关键词并检索 arXiv 文献 | Refine 按钮 | 仅 LLM 单轮（Mock 模式可用） |
...
`useMock=True` 时与 Plan 对齐：从 `test/mock-ai/refine.json` 加载 mock，使用 `mock_chat_completion` 流式输出 reasoning 到 Thinking。
